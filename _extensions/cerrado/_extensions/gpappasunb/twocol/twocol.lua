...
local function process_attributes(attrs)
	columns_attrs = {
		-- left = {{align="top"},{width="20%"}},
		-- right = {{align="top"},{width="80%"}}
		left = {},
		right = {},
	}
	...
	for _, attr_name in ipairs({ "align", "width" }) do
		if attrs[attr_name] then
			-- print("Processing " .. attr_name)
			-- Split the attribute string. ("left,right") -> { left = "left", right = "right" }
			local split = split_attributes(attrs[attr_name])
			-- print("split.left=" .. split.left)
...
function Div(div, attrs)
	...
	local left_attrs = pandoc.Attr("", { "column", "left-content" }, {})
	if columns_attrs.left then
		left_attrs = pandoc.Attr("", { "column", "left-content" }, table.unpack(columns_attrs.left))
		-- print(table.unpack(columns_attrs.left))
		-- dumpTable( columns_attrs.left)
	end

	local right_attrs = pandoc.Attr("", { "column", "right-content" }, {})
	if columns_attrs.right then
		right_attrs = pandoc.Attr("", { "column", "right-content" }, table.unpack(columns_attrs.right))
	end
